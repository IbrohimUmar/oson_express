...
@permission_required('admin.seller_app_order_details', login_url="/home")
def seller_app_order_details(request, id):
    seller = get_seller(request.user)
    order = get_object_or_404(Order, id=id, status__in=[3, 4, 5, 6], seller=seller, driver__isnull=False)
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                return redirect("seller_app_order_details", id)
        except IntegrityError:
            messages.error(request, "Sizda xatolik mavjud")
            return redirect('seller_app_order_list')
    return render(request, 'seller_app/orders/details.html',
