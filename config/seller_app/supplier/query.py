...
class SupplierData(object):
    ...
    def total_buy_products_amount(self):
        ...
        stock = WareHouseStock.objects.filter(action_type='1', warehouse_operation__supplier=self.user,
                                              warehouse_operation__status='2').aggregate(t=Coalesce(Sum(ExpressionWrapper(F("input_price") * F("quantity"), output_field=models.IntegerField())), 0))['t']
        return stock
