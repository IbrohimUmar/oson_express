...
def seller_app_supplier_edit(request, id):
    ...
    if request.method == "POST":
        ...
        if not phone:
            ...
            return render(request, 'seller_app/supplier/edit.html', {"r": request.POST, 'messages_error':"Telefon raqam kiritilmagan"})
        if len(r['password_text'])<8:
            return render(request, 'seller_app/supplier/edit.html', {"r": request.POST, 'messages_error':"Parol uzunligi kamida 8 tadan ko'p bo'lishi kerak. xafsizlik yuzasidan"})
        if User.objects.filter(username=int(r['username'])).exclude(id=id).exists():
            return render(request, 'seller_app/supplier/edit.html', {"r": request.POST, 'messages_error':"Bu telefon raqamli foydalanuvchi mavjud. iltimos boshqa telefon raqam kiriting"})
        ...
        return redirect('seller_app_supplier_edit', id)
