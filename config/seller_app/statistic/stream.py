...
@permission_required('admin.seller_app_statistic_stream', login_url="/home")
def seller_app_statistic_stream(request):
    streams = SellerStream.objects.filter(seller=request.user).order_by("-id")
    ...
    return render(request, 'seller_app/statistic/stream.html', {'quary': filter_query,'page_obj': pagination_queryset, 'page_streams':page_streams, 'count': streams.count(),
                                                          })
