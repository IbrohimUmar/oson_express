...
@permission_required('admin.seller_app_statistic_order', login_url="/home")
def seller_app_statistic_order(request):
    orders = Order.objects.filter(seller=request.user).order_by("-updated_at")
    ...
    return render(request, 'seller_app/statistic/order.html', {'statuses':Status, 'quary': filter_query,'page_obj': order, "order": order, 'count': orders.count(),
                                                          })
