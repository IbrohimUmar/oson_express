...
def warehouse_product_attachment_print(request):
    region = Regions.objects.all()
    products = cache.get_or_set("product_list_cache", Product.objects.all())
    return render(request, 'warehouse/product_attachment/print/print.html', {"region":region, "products":products})
    # return render(request, 'order/print/test.html', {"region":region, 'products':products})


@login_required(login_url='/login')
@permission_required('admin.order_print_unshipped_orders', login_url="/home")
def warehouse_product_attachment_packaging_order_print(request):
    region = Regions.objects.all()
    products = cache.get_or_set("product_list_cache", Product.objects.all())
    return render(request, 'warehouse/product_attachment/print/packaging_order_print.html', {"region":region, "products":products})




@permission_required('admin.warehouse_product_attachment_history', login_url="/home")
def warehouse_product_attachment_print_api(request):
    ...
    order_product = OrderProduct.objects.filter(order__status=status, product_type__in=[1, 2]).values("product_id", "product__name").annotate(
    ...
    if request.method == "POST":
        ...
        Order.objects.filter(id__in=body['orders_id']).update(status=8)
        return JsonResponse({
            'status': 200,
        })
    order_list = []
