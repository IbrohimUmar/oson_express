...
def order_print_unshipped_orders(request):
    region = Regions.objects.all()
    products = cache.get_or_set("product_list_cache", Product.objects.all())
    return render(request, 'orders/print/print_unshipped_orders.html', {"region":region, "products":products})
    # return render(request, 'order/print/test.html', {"region":region, 'products':products})


@permission_required('admin.order_print_unshipped_orders', login_url="/home")
def print_unshipped_orders_api(request):
    
    orders = Order.objects.filter(status=1, barcode__isnull=False, orderproduct__product_variable__isnull=False).order_by("id")
