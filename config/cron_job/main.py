...
def cronjob_main(request, code, type):
    if str(code) == CRONJOB_TOKEN and str(type) == 'db_backup' or str(type) == 'driver_balance_save':
        if str(type) == 'db_backup': # 24 soatda 1 marta
            backup_database()
            return JsonResponse({"status": 200, 'message': "Olindi"})
        if str(type) == 'driver_balance_save': # har 1 soatda
