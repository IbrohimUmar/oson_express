...
@permission_required('admin.operator_date_by_statistic', login_url="/home")
def operator_date_by_statistic(request):
    now = timezone.now()
    ...
    total_counts = Order.objects.filter(updated_at__date__range=(from_date, to_date)).aggregate(
    ...
    driver = User.objects.filter(type='3', is_active=True).annotate(
    ...
    return render(request, 'operator/date_by_statistic.html', {"driver" :driver, 'now' :now.strftime("%Y-%m-%d"), 'total_results' :total_counts})
