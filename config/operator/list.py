...
@permission_required('admin.operator_list', login_url="/home")
def operator_list(request):
    #    operator = User.objects.filter(type=3, is_active=True).order_by("is_active")
    operator = User.objects.filter(type=3).order_by("-id")
    ...
    return render(request, 'operator/list.html', {'page_obj': queryset, 'count': operator.count(),
