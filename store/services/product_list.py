...
class ProductListService:
    def get_product_json_by_site(self, site=1):
        all_products = [{'id':i['id'], 'name':i['name'], 'price':i['sale_price'], 'toll':self.get_toll_amount(i['toll'])} for i in Product.objects.filter(is_active=True).values("id", 'name', 'sale_price', 'toll')]
        return all_products
